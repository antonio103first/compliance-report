...
class InvestmentReportData:
    ...
    corp_reg_number: str = ""          # 법인등록번호 (별첨2용, 통상 외부조회/수동주입)
...
def _scan_all_tables(tables, data: InvestmentReportData):
    ...
    for table in tables:
        ...
        # ── 회사명 fallback (축약 보고서: '투자업체'/'투자업체 명' 행) ──
        if not data.company_name:
            _extract_company_name_fallback(table, data)
...
def _extract_company_name_fallback(table, data: InvestmentReportData):
    """축약 투심보고서: 회사 개요 테이블이 없어 '투자업체'/'투자업체 명' 행에서 회사명 추출."""
    _LABELS = ('투자업체', '투자업체명', '투자기업', '투자기업명',
               '투자대상', '투자대상기업', '투자대상기업명', '대상기업', '대상기업명')
    for row in table.rows:
        cells = [c.text.strip() for c in row.cells]
        for i, c in enumerate(cells):
            label = c.replace(' ', '')
            if label in _LABELS and i + 1 < len(cells):
                val = cells[i + 1].strip()
                # 값 자체가 라벨이거나 펀드명/조합명이면 제외
                if val and val.replace(' ', '') not in _LABELS \
                        and '투자조합' not in val and '펀드' not in val:
