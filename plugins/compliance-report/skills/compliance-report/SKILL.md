...
- **이노비즈/메인비즈**: 혁신형중소기업현황(3033893) zip을 **자동 다운로드**(인증키 불필요) →
  ...
  - **메인비즈**: 이노비즈넷 같은 공개 인증(유효기간) 조회가 **없다**(공식 확인은 중소벤처24 기업회원
    로그인 필요). 따라서 인증 확정은 로컬 캐시(2024-06-30)로 하고, 미확정 시 **메인비즈협회 회원사 조회**
    (`find_member.asp`)를 **실시간 '힌트'로만** 표시한다 — *회원 ≠ 인증* 이라 자동 적(Y)로 확정하지 않고
    담당자가 중소벤처24에서 최종 확인하도록 안내한다.
...
#### 특약사항 한줄 요약 — `--special-terms`

계약서 **본문(제1~N조)** 에 특약사항(특별약정 등) 조항이 있으면 **한 줄로 간결히 요약**하여 표2
주요투자조건의 ` - 기타` 에 `특약사항: …` 로 넣는다. **뒤쪽 별지/별첨(진술과보장·투자금용도·
퇴사제한·경업금지약정 등)은 특약사항에서 제외**한다.

- 인자 미지정 시: 계약서 특약 조항을 자동 추출·압축(룰 기반, 상투구 제거).
- **스킬로 호출할 때는 Claude 가 직접** 특약 조항을 읽고 **아주 간결한 1줄**로 요약해 `--special-terms`
  로 전달한다. 예: `--special-terms "투자금 납입후 3년내 코스닥 상장 추진, 미이행시 동반매도권 부여"`
  (주어·군더더기 빼고 핵심 의무/효과만, 별지 약정은 제외).
