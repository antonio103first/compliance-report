...
- **이노비즈/메인비즈**: 혁신형중소기업현황(3033893) zip을 **자동 다운로드**(인증키 불필요) →
  ...
  - ⭐ **이노비즈넷 실시간 보완**: 위 파일은 갱신이 느려(현재 2024-06-30 기준) 최근 인증을 놓칠 수
    있다. 로컬 캐시가 확정하지 못하면(미발견/만료/모호) **이노비즈넷 실시간 조회**(`ajax.fill_com.asp`,
    순수 HTTP·로그인 불필요)로 자동 보완해 현재 유효 인증·유효기간을 확인한다. (`CERT_WEB_FALLBACK=0` 으로 비활성)
...
> 미발견은 비워 두고 경고를 남겨 담당자가 확인한다. **애매한 매칭 시 경고에 회사명으로 사전 필터된
> 공개조회 딥링크**(벤처공시 `?cmpNm=`, 이노비즈넷/메인비즈)를 포함한다. 매 실행 시 **자료 기준일**도
> 표시한다(벤처 명단은 월 갱신; 혁신형중소기업은 data.go.kr 갱신 주기에 의존 — 현재 2024-06-30 기준).
...
| `--corp-enrich` | **사업자등록번호(+회사명)** 로 금융위 기업기본정보 API 조회 → 설립일·법인등록번호·대표자·주소·업종 보완 (**표1·표6 + 창업기업 7년 자동판정**). 환경변수 `DATA_GO_KR_CORP_KEY` |
